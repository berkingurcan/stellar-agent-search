...
check(
  pkg.bin?.["stellar-agent-search"] === "dist/index.js" && Object.keys(pkg.bin).length === 1,
  "package.json must expose exactly the stellar-agent-search CLI bin",
);
...
check(
  !smithery.includes('args: ["-y", "stellar-agent-search"]'),
...
for (const path of persistentDocs) {
  ...
  check(
    !/npx\s+(?:--yes|-y)\s+stellar-agent-search(?=[\s"'`]|$)/.test(contents),
    `${path} contains an unpinned persistent npx launch`,
  );
  check(
    !/npm\s+(?:i|install)\s+(?:-g\s+)?stellar-agent-search(?=[\s"'`]|$)/.test(contents),
...
check(
  publishedLanding.includes("packageMetadata.version") && !publishedLanding.includes("stellar-agent-search@0.1.0"),
