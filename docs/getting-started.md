...
> [official package page](https://www.npmjs.com/package/stellar-agent-search) shows version 0.1.0, do not run an
> `npx stellar-agent-search` command. The setup flow below pins `@0.1.0` into persistent client configuration
> after the official package exists.

`stellar-agent-search` is a single Node binary with three entry points:
...
npx -y stellar-agent-search@0.1.0 find "scraper" --x402

# Full profile for a specific agent (declared reputation + contract-probe limits, capabilities, services)
npx -y stellar-agent-search@0.1.0 profile 10

# Rank a query's candidates or an explicit id set; attempted contract probes fail closed
npx -y stellar-agent-search@0.1.0 rank "scraping agents"
npx -y stellar-agent-search@0.1.0 rank 10 12 15

# Catalog of self-declared service endpoint candidates
npx -y stellar-agent-search@0.1.0 services --x402

# Machine-readable output for any command
npx -y stellar-agent-search@0.1.0 find "scraper" --json
```
...
All three register the same version-pinned stdio launch: `npx -y stellar-agent-search@0.1.0 mcp`. Existing matching entries
...
`npx -y stellar-agent-search@0.1.0 mcp` launch so the same registration works without a prior global install. You can
...
The fixed `stellar-agent-search-declared-evidence-v1` rank policy normalizes quality against exactly `100` and
